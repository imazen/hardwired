      ...
      def load(path, app)
        Dir.chdir(Hardwired::Paths.root || '.') do
          Dir.glob(path) do |file|
            ...
            @config = NormalizingRecursiveOpenStruct.new(config_for_env(default_config.merge(YAML.safe_load(document, permitted_classes: [Symbol, Date, Time]) || {}),app.environment) || {})
            return @config 
          end
